...
public class BankingService {
	/**
	 * 계좌정보에 대한 추가/조회/삭제/변경 기능을 제공하는 객체를 참조한다. 
	 */
	...
	public Account getAccountDetail(int no,int password) {
		Account account = repo.getAccount(no);
		if(account == null) {
		...
		return account; // 위 두 경우를 만족하지 않는 경우 계좌정보를 제공한다!
		
	}
	/**
	 *  계좌버놓, 입금액을 전달받아서 계좌의 현재 잔액을 증가시킨다.
	 * @param no 계좌번호
	 * @param amount 입금액
	 */
	...
	public void withdraw(int no, int password, long amount) {
		...
		if(account.getBalance() < amount) {
			...
			throw new NotEnoughBlanceException(message);
		}
		account.setBalance(account.getBalance() - amount);
